...
In epidemiology, we need to understand how infections spread through a population over time.
Current infections are caused by past infections, and this transmission process forms the foundation of epidemic dynamics.
We want to link infections themselves together over time, knowing that current infections were infected by past infections.
...
When we observe surveillance data like symptom onsets or positive tests, we often want to understand the underlying infection process that generated these observations.
A simple approach might be to assume that infections every day are independently and identically distributed, determined only by the observed data.
However, in reality, we know that infections are not independent.
...
The renewal equation represents a convolution of the infection time series with itself, with the delay distribution given by $g_i$ and $R_t$ representing a scaling that is being applied. This convolution structure captures how past infections contribute to future infections through transmission.
...
This discrete formulation can be interpreted as a discretised version of a continuous renewal equation where the sum is replaced by an integral and the generation time distribution is continuous.
...
This follows a mathematical convolution pattern where we're combining two time series - the infections and the generation time distribution.
...
Let's now work with a simulated time series of infections to demonstrate the renewal equation in practice.
...
We can use the `censored_delay_pmf()` function to discretise this continuous distribution for use with the discrete renewal equation. This function handles the conversion from continuous to discrete probability distributions.
...
We first load the stan model and spend some time trying to understand it.
...
We can simulate observed data (like symptom onsets) from a time series of infections by convolving with a delay distribution and then sampling from a Poisson distribution. This captures both the delay process and observational noise.
...
Familiarise yourself with the model above.
Compare it to the simpler model introduced in the slides.
...
A simpler modelling approach would assume that the number of infections each day is independent and identically distributed.
...
Compare the results across the different models used in this session.
